...
export const DYNAMIC_KEY_PREFIXES: DynamicPrefix[] = [
  ...
  // ── director(导演台 V2)──
  { prefix: 'director.topbar', why: '动态: 顶栏工具名;枚举来源: TopBar 的 TOOL_KEYS(select/translate/rotate/scale → topbar.* 词条)' },
  { prefix: 'director.cameraPreset', why: '动态: 机位预设 id;枚举来源: model/cameraPresets.ts 的 CAMERA_PRESETS[].id(cameraPreset.* 词条)' },
  { prefix: 'director.lightType', why: '动态: 灯光类型;枚举来源: model/directorTypes.ts 的 LightType 联合(directional/point/spot)' },
  { prefix: 'director.creation.lightName', why: '动态: 新建灯光默认名;枚举来源: LightType 联合(creation.lightName.* 词条,带 {{index}})' },
  { prefix: 'director.primitive', why: '动态: 基础几何体类型;枚举来源: model/directorTypes.ts 的 PRIMITIVE_TYPES(primitive.* 词条)' },
  { prefix: 'director.assets.folder', why: '动态: 资产库文件夹 id;枚举来源: AssetsTab 的目录（linked/uploads/splats/models/lights/primitives）' },
  { prefix: 'director.assets.kind', why: '动态: 资产类型;枚举来源: directorTypes 的 DIRECTOR_ASSET_KINDS(model/splat/panorama/scene)' },
  { prefix: 'director.displayMode', why: '动态: 模型显示模式;枚举来源: ModelDisplayModeSwitch 的 MODES(solid/translucent/clay,含 *Hint 提示)' },
  { prefix: 'director.lens', why: '动态: 焦段预设 id;枚举来源: model/cameraLens.ts 的 LENS_PRESETS[].id(lens.* 词条)' },
  { prefix: 'director.colorPreset', why: '动态: 颜色色板 id;枚举来源: scene/sceneTheme.ts 的 CHARACTER_COLOR_PRESETS 与 model/lights.ts 的 KELVIN_PRESETS(colorPreset.* 词条)' },
  { prefix: 'director.lightOrientation', why: '动态: 灯光快捷朝向 id;枚举来源: model/lights.ts 的 LIGHT_ORIENTATION_PRESETS[].id(lightOrientation.* 词条)' },
  { prefix: 'director.timeline.family', why: '动态: 时间轴副轨家族;枚举来源: model/timelineTracks.ts 的 TrackFamily(trajectory/action/bone/lookat/closeup)' },
  { prefix: 'director.timeline.menu', why: '动态: 时间轴右键菜单项;枚举来源: timeline/DirectorTimeline.tsx buildMenu 的 label(key) 调用(menu.* 词条)' },
  { prefix: 'director.closeup.anchor', why: '动态: 特写锚点;枚举来源: model/directorTypes.ts 的 CloseupAnchor' },
  { prefix: 'director.closeup.facingMode', why: '动态: 特写朝向模式;枚举来源: model/directorTypes.ts 的 CloseupFacingMode' },
  { prefix: 'director.closeup.azimuthName', why: '动态: 特写方位;枚举来源: model/directorTypes.ts 的 CloseupAzimuth' },
  { prefix: 'director.closeup.motionName', why: '动态: 特写运镜预设;枚举来源: model/directorTypes.ts 的 CLOSEUP_MOTION_PRESETS' },
  { prefix: 'director.character.handle', why: '动态: IK 把手名;枚举来源: model/ikChains.ts 的 IkHandleKey' },
  { prefix: 'director.character.joint', why: '动态: FK 关节名（左右各自命名）;枚举来源: model/rigs.ts 的 FK_JOINTS 展开左右' },
  { prefix: 'director.character.jointShort', why: '动态: 2D 人偶端点短名;枚举来源: panels/inspector/PuppetFigure.tsx 的带名关节' },
  { prefix: 'director.character.handleShort', why: '动态: 2D 人偶把手短名;枚举来源: model/ikChains.ts 的 IkHandleKey' },
  { prefix: 'director.joint', why: '动态: 关节三轴语义标签;枚举来源: model/rigs.ts 的 jointAxisLabelKey(FK_JOINTS × xyz)' },
  { prefix: 'director.bodyType', why: '动态: 快捷体形 id;枚举来源: model/rigs.ts 的 BODY_TYPE_PRESETS[].id' },
  { prefix: 'director.action.library', why: '动态: 动作库条目;枚举来源: model/actionLibrary.ts 的 ACTION_LIBRARY[].id' },
  { prefix: 'director.lookAtClip.targetType', why: '动态: 视线目标类型;枚举来源: model/directorTypes.ts 的 LookAtTargetType' },

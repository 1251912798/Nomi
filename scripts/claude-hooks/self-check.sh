...
① 动手写码前 —— 想清楚再动(P5)：用户可见? 先读 docs/design/nomi-design-system.md + 出可体验样张 + 用户拍板(R8)，改/扩现有 UI 先看真实样子=读完整外壳组件或截图，加/挪控件先过 §1.5 控件层级｜碰三方库/新框架? 先 Context7+web 查最现役(R5)｜碰框架/SDK/运行时(含它没用过的层)? 先出四列表「它提供/我们用了/我们另写了/我们拆散了」+ 参考实现逐层对照「按九层拆开它自带的 coding agent/官方 example，判定 一致·有意不同(理由须是领域约束)·没想到」(R29)｜碰外部也读写的格式/协议/契约(技能包·MCP 配置·模型可见工具 schema·导入导出·供应商 API)? **先对齐标准再动手**：找规范→写「规范链接/我们的偏差/偏差理由(只许领域约束不许偏好)」→扩展只放标准的扩展点、不许另起平行文件(R31)｜多文件? 先写 docs/plan(R4)｜取舍/架构岔路? 给对比表让用户拍板(R3)
② 报完成/交付给用户看前 —— 全绿≠完成(P3)：和获批样张逐项对账 + 真机走查(R13)，截图必须自己亲眼 Read 过，来自用户将跑的那个构建/平台/入口，拍得到改动区。没闭环就别说「做完/修好/通过/给你看」｜Agent/工具/契约改动? 要有真实模型数字：工具写对率+回合成功率(R30)，实验室基线只证外观、截图只证界面在，都不证明按下去会发生事

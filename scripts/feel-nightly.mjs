...
import { compareFeelBaseline } from '../tests/ux/_feel-observer.mjs'
...
export async function runFeelNightly({ journeyId } = {}) {
  ...
  if (records.some((record) => record.drift.length)) throw new Error(`Feel baseline drift; inspect artifacts/feel/${reportName}.json`)
  return report
}

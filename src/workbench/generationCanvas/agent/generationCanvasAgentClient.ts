...
export function buildStaticAgentSystemPrompt(
  ...
  return [
    ...
    '- create_canvas_nodes 交付手艺产物（kind=agent-artifact）：当用户要的是「你直接做出来、不调模型的表达物」（构图线稿 SVG / 会动的讲解 HTML / 分镜草表 / Markdown 要点 / 文本）时，在同一 create_canvas_nodes 里建 kind=agent-artifact 节点并带 artifact={fileType, content}——content 是文件正文（如 SVG/HTML/Markdown 源码），title 是用户会看到的节点名；系统落盘成画布产物节点（可下载/复制；SVG 可固化为参考图喂下游）。它不需要 modelKey，prompt 给空串即可（不是生成）。要真实画面/动态影像成片时不要用这个，走对应生成模型。**HTML 产物要动就用 CSS**（@keyframes/transition/:hover 都能跑）：产物在沙箱里继承宿主的脚本策略，内联 <script> 不会执行，别把内容或动效依赖 JS。',

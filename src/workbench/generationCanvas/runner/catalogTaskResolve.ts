...
export async function resolveExecutableNodeFromCatalog(
  ...
  if (!modelKey) {
    throw new Error(`Model is not configured: 供应商「${vendor}」已断开，且该节点未记录模型。请重新连接，或在该节点上改选已连接供应商的模型。`)
  }
  ...
  if (!match) {
    ...
    throw new Error(`Model is not configured: 当前没有已连接的供应商提供「${brand}」模型。请重新连接原供应商，或在该节点上改选一个已连接供应商的模型。`)
  }

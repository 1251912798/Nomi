export const zhOnboardingProviders = {
  ...
  assistant: {
    ...
    // pi 自己不带 MCP，得先装社区适配器才读得到我们写的配置。命令只给不代跑。
    piAdapterTitle: 'Pi 还需要装一次 MCP 适配器',
    piAdapterBody: 'Pi 本身不带 MCP。在终端里跑一次下面这条命令装上适配器，它会自动读到 Nomi 写进 ~/.config/mcp/mcp.json 的配置；装完重开 Pi。',
    piAdapterCopy: '复制命令',
  ...
  localModel: {
    ...
    detectingInline: '检测中…',
    redetect: '重新检测',
    notDetectedTitle: '没检测到本地模型服务',
    notDetectedBody: '确认 Ollama（11434）、LM Studio（1234）或 LocalAI（8080）已在本机启动，再点「重新检测」。',
    connect: '连接',
    disconnect: '断开',
    allConnected: '这台的模型都已连上',
    noModelsLoaded: '这个服务在跑，但还没加载任何模型',
    capability: {
      agent: '支持 Agent',
      chatOnly: '仅对话',
      unknown: '能力未知',
...
export const enOnboardingProviders = {
  ...
  assistant: {
    ...
    piAdapterTitle: 'Pi needs its MCP adapter first',
    piAdapterBody: 'Pi ships without MCP. Run the command below once in a terminal to install the adapter; it picks up the configuration Nomi writes to ~/.config/mcp/mcp.json on its own. Restart Pi afterwards.',
    piAdapterCopy: 'Copy command',
  ...
  localModel: {
    ...
    status: {
      ...
      detecting: 'Detecting…',
      detected: 'Local service detected',
      notDetected: 'Not detected',
    },
    detecting: 'Detecting local services…',
    detectingInline: 'Detecting…',
    redetect: 'Re-detect',
    notDetectedTitle: 'No local model service detected',
    notDetectedBody: 'Make sure Ollama (11434), LM Studio (1234) or LocalAI (8080) is running on this machine, then re-detect.',
    connect: 'Connect',
    disconnect: 'Disconnect',
    allConnected: 'All models on this service are connected',
    noModelsLoaded: 'This service is running but has no models loaded yet',
    capability: {
      agent: 'Agent-ready',
      chatOnly: 'Chat only',

...
it('profiles never author derived consumesAnchors and all resolved modes agree with slots', () => {
  const files = [...sources('src/config/modelArchetypes'), ...sources('electron/shared/videoCapabilities')]
  for (const file of files.filter(file => !file.endsWith('/index.ts') && !file.endsWith('/types.ts'))) {
    expect(readFileSync(file, 'utf8'), file).not.toMatch(/\bconsumesAnchors\s*:/)
  }
  for (const profile of MODEL_ARCHETYPES) {
    const resolved = resolveArchetypeForModel({ modelKey: profile.identifierPatterns[0], meta: { archetypeId: profile.id } })!
...
it('shot/node precedence is owned by effectiveShotValue, not inline fallbacks', () => {
  for (const file of sources('src/workbench')) {
    expect(readFileSync(file, 'utf8'), file).not.toMatch(/node\??\.prompt\s*(?:\?\?|\|\|)\s*shot\.prompt/)
  }
})

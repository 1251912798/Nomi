// Stage-02-only experiment. One persisted ¥2 reservation ledger spans all three samples.
...
export function quotePlanSample(prices, model) {
  ...
  if (pricing?.unit !== 'usd_per_million_tokens' || pricing?.tier_count !== 1
    || !(pricing.rates?.input > 0) || !(pricing.rates?.output > 0)) throw new Error('C0_SAMPLE_PRICE_UNKNOWN')
  return { planOnly: true, models: { ...REAL_MODELS, text: model }, rates: pricing.rates,
    maxOutputTokens: Math.min(pricing.limits?.max_output_tokens ?? 8192, model === 'gpt-5-nano' ? 16000 : 8192), budgetCny: 2, source: 'https://apimart.ai/pricing', checkedAt: new Date().toISOString() }
}
export const planSampleFetch = ({ send, quote, ledger, persist }) => async (input, init) => {
  ...
  if (!Number.isFinite(reservedCny) || reservedCny > quote.budgetCny) throw new Error('C0_BLOCKED_BUDGET')
  ...
  persist()
  try {
    const response = await send(input, { ...init, body: JSON.stringify(body), redirect: 'error' })

...
if (values.help) {
  ...
  console.log('--real currently fails closed at the runbook budget gate; no verified all-in H3 quote is bundled.')
  process.exit(0)
}
if (Boolean(values['dry-run']) === Boolean(values.real)) throw new Error('Select exactly one of --dry-run / --real')
if (values.packaged && !path.isAbsolute(values.packaged)) throw new Error('--packaged requires an absolute path')
...
const report = {
  mode: values.real ? 'real' : 'dry-run', sourceSha: sha, platform: `${os.platform()} ${os.arch()}`,
  executor: 'Codex', reviewer: 'pending', attemptDir, inputSha256: hash(scriptFile),
  paidCalls: 0, costCny: 0, budgetCny: values.real ? 5 : 0, c0Accepted: false,
...
// Do not read user settings or expose keys merely to discover that the budget is unknown.
// The runbook explicitly requires blocked-budget before dispatch when all-in pricing is unknown.
if (values.real) {
  report.result = 'blocked-budget'
  report.blocker = 'No verified Agent + 64s MiniMax H3 + retry/in-flight cost upper bound <= CNY 5. Real dispatch is not implemented; prepare a candidate packaged build and verified quote first.'
  save()
  console.error(report.blocker)
  process.exit(1)
}
let app, win, fixture, screenshotSettled, expect, stopRuntimeApp
...
try {
  ...
  const { DOCUMENT, CREATION_PANEL, APPROVAL_CARD, INTERVENTION_CONFIRM,
    readProject, hasToolResult, recorded, openCanvas } = support
  ;({ stopRuntimeApp } = support)
  const { createC0Fixture, shots, MODEL } = await import('./c0-fixture.mjs')
  const { flattenRequestText } = await import('../agent-runtime-fixture.mjs')
  const tempRoot = path.join(attemptDir, 'profile')
  const settingsDir = path.join(tempRoot, 'settings')
  fixture = await createC0Fixture(root, settingsDir, path.join(attemptDir, 'fixture-media'))
  ...
  const launch = () => launchNomiApp({ name: 'g1-c0', tempRoot, settingsDir, settleMs: 0,
    ...(executablePath ? { executablePath } : {}),
    env: { NOMI_RENDERER_URL: '', VITE_DEV_SERVER_URL: '', NOMI_DESKTOP_DEV: '',
  ...
  const planId = 'c0-plan-1'
  let plan, done
  await step('02', '拆分并审阅八镜分镜', '真实提案审批，八镜共64秒，尚无付费调用', async () => {
    ...
    await recorded(plan.received, 'C0 planner')
    report.r30.simulated.firstTool = '0/1 (0%; result not yet verified)'
    ...
    await expect(win.locator(CREATION_PANEL)).toContainText('C0_PLAN_DONE')
    ...
    expect(actual.map((s) => s.prompt)).toEqual(shots.map((s) => s.prompt))
  ...
  const reviews = []
  await step('03', '分镜物化到画布', '八个节点顺序、参数对应；生成前仍零请求', async () => {
  ...
  await step('04', '确认并生成八段模拟视频', '八个不同请求，各自产物落为项目本地视频；禁止重复请求', async () => {
    ...
    await clickOrFail(spendDialog.getByRole('button', { name: '生成', exact: true }), '批准零额度生成')
    ...
    await Promise.all(reviews.map((review) => recorded(review.received, 'C0 synthetic review')))
  ...
  await step('07', '冷重启检查资产和时间轴', '同一项目八个资产和八段剪辑均保留；完整看片仍须人眼签收', async () => {
    ...
    report.review = 'Pending human inspection: synthetic motion/audio tests plumbing only, not story/identity/continuity quality.'
  })
  report.result = 'dry-run-assertions-passed-review-pending'
} catch (error) {
  ...
  console.error(`C0 failed: ${error.message}`)
  process.exitCode = 1
} finally {
  try { if (app) await stopRuntimeApp(app) } catch { report.result = 'cleanup-failed'; process.exitCode = 1 }
  try { if (fixture) { await fixture.close(); fixture.text.assertClean() } } catch { report.result = 'fixture-failed'; process.exitCode = 1 }
  save()

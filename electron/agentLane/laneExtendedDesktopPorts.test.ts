...
const plan = { planId: 'plan-1', baseRevision: 'revision-1', summary: 'Move clip', operations: [{ kind: 'move', clipId: 'clip-1', startFrame: 0 }] }
...
function setup() {
  ...
  const input: LaneExtendedDesktopPortsInput = {
    ...
    production: { tryExecute: vi.fn(async () => ({ ok: true as const, result: { runId: 'run-1' } })),
      prepare: vi.fn(async () => null), execute: vi.fn(async () => ({ ok: false as const, code: 'unreachable', message: 'unreachable' })), dispose: vi.fn() },
...
describe('deferred desktop domain authority', () => {
  it('never executes an unprepared or unapproved timeline write', async () => {
    const f = setup(), value = call('apply_edit_plan')
  ...
  it.each(['export_timeline', 'delete_canvas_nodes'])('requires the correlated committed G5 receipt for %s', async name => {
    const f = setup(), value = call(name, name === 'export_timeline' ? { expectedRevision: 'revision-1' } : { nodeIds: ['node-1'] })
    await f.prepareAndApprove(value)
  ...
  it('accepts export only when its exact approved receipt is committed', async () => {
    const f = setup(), value = call('export_timeline', { expectedRevision: 'revision-1' })
    await f.prepareAndApprove(value)
  ...
  it('joins a successfully created draft to the domain task projection', async () => {
    const f = setup(), value = call('start_production_run', { goal: 'Create a short film' })
    await f.prepareAndApprove(value)
    expect(await f.execute(value)).toMatchObject({ ok: true })

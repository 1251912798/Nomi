...
| **窗口 chrome / 全屏浮层** | Windows 是 `frame:false` + 渲染层自绘 32px 窗口栏（`WorkbenchShell` 里那条 `app-drag`，唯一一份 `WindowControls`）；mac/Linux 交系统。全屏浮层（导演台 / 白板 / ComfyUI 整页 / 全景 / 素材预览）**从窗口栏之下起画**，顶偏移只来自 `currentFullscreenOverlayTopOffset()` | `src/ui/app-shell/windowChrome.ts`、`src/ui/app-shell/WindowControls.tsx`、`src/workbench/WorkbenchShell.tsx:255` | ❌「浮层 `fixed inset-0` 铺满就行，反正 z-index 更高」——**拖拽区按几何做原生命中测试，不看 DOM 层级**：盖住那 32px，浮层顶部一条的点击会被系统当成拖窗口吃掉，窗口控件也埋在下面（issue #58 与 2026-09-04 导演台顶栏点不动同根，见 `docs/fixes/2026-09-04-fullscreen-overlay-windows-windowbar.root-cause.json`） |
...
| **3D 导演台** | 只有一个 3D 节点 `director`（`nodes/director/`，完整的 3D 导演台：视口 / 时间轴 / 机位 / 角色骨骼 / 环境 / 出片 / AI 搭场景 / 手机虚拟相机）。老 `scene3d` 节点在画布快照加载时经 `nodes/director/migration/` 单向迁成它；AI 来导（`create_staging_reference` / `create_camera_move`）由 `nodes/director/agent/` 建 `director` 节点 + 常驻 Host 离屏出图 / 出片 | `docs/plan/2026-09-02-director-console-v2.md`、`docs/plan/2026-09-03-director-cutover-gate.md` | ❌ 以为还有 V1 `nodes/scene3d/` / `Scene3DEditor`（2026-09-03 切换门已删）；❌ 把 `scene3d` 当现役节点类型 |

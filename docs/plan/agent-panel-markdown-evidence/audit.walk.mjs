...
try {
 ...
 for(const theme of ['light','dark']){
  ...
  for(const [id,prompt,text] of cases){
   ...
   if(id==='02-code'){
    await md.getByRole('button',{name:'复制代码',exact:true}).click()
    const copied=await app.evaluate(({clipboard})=>clipboard.readText())
    evidence.at(-1).copy={expected:code+'\n',actual:copied,match:copied===code+'\n'}
    fs.writeFileSync(path.join(out,'observations.json'),JSON.stringify(evidence,null,2))
   }
   if(id==='06-storyboard'){
    const more=md.getByRole('button').filter({hasText:/还有/})
    if(await more.count()){
     await more.click()
     await snap('06-storyboard-expanded',theme,md.locator('strong').last())
    }
   }
  }
  await newConversation(win,CREATION_PANEL)

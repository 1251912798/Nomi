...
## 用户价值：这不是预检闸，是 Agent 写分镜时的决策工具（2026-09-07 用户原话）

> 上一版方案通篇把它写成「落画布前的合法性预检」——那是**实现视角**。用户 2026-09-07 说清了它真正
> 是什么：**Agent 在写分镜的那一刻，需要一个懂模型的副驾**。差别不是措辞，是取舍顺序：预检的目标
> 是「别让非法请求出门」，决策工具的目标是「这一镜怎么拍最像人做的」——后者要在方案期就影响
> 模式、参数与镜头结构，而不是等方案定稿再来挑错。

用户举的三件事（D1：从他那一刻的真实摩擦出发，不从我们的模块边界出发）：

| 用户那一刻卡在哪 | 他要的判断 | 现在引擎给不给得出 |
|---|---|---|
| 「直接文生视频，人脸和服装会乱变」 | **该用参考图模式锁人**（跨镜同角色 → `character` / `image_ref`；同场承接 → 首尾帧） | ❌ **只做了校验，没做建议**——见下方「已覆盖 / 未覆盖」 |
| 「这镜要不要音频？分辨率、秒数怎么定？」 | 每镜的参数按模型档案真实档位定下来，不合法的当场钳并说明 | ✅ 已覆盖（`param.*` / `duration.*` 判据 + 面板逐条「为什么」） |
| 「前两镜合计 10 秒没超单条上限 → 并成一条整体生成最优；第三镜 40 秒超限 → 得拆」 | 计划级的**结构调整**：短拍合并、超限拆条 | ✅ 已覆盖（`mergeProposals` / `splitProposals`，可逐条采纳） |

**为什么这个定位改变了 UI 的形状**：既然它是写分镜时的副驾而不是出门前的安检，判据就不能只住在
一个「点开才看得见的面板」里——所以本次返工把同一批判据同时画在**分镜行上**（返工 7 的行内警示）：
摩擦发生在哪一行，提示就在哪一行。面板负责「整份方案的结构建议 + 逐条采纳」，行负责「这一镜有问题」。

### 建议类型覆盖表（诚实标缺口，D4）

引擎当前产出两类东西：**校验判据**（`PlanIssueCode` 11 条：模型/模式/参数/时长）与**结构建议**
（`MergeProposal` / `SplitProposal`）。按用户这三件事对账：

- ✅ **时长与参数**：钳值、枚举回落、超上限/低于下限，全部有 code + 真实数值 + 人话。
- ✅ **时长合并 / 拆条**：含拆条默认建议首尾帧承接（`suggestFirstLast`，与通行解法对齐，见「先查别人」）。
- ❌ **模式选择建议（参考图 / 文生 / 首尾帧）**：**本 PR 不做**。引擎现在只在「你指名的 modeId 档案里没有」
  时回落并报 `mode.fallback`——那是**校验**，不是**建议**。用户真正要的「这几镜出现同一个角色 → 建议改走
  参考图模式锁脸锁衣」需要的判据（跨镜锚点复现、锚点有没有定妆图、模式槽位能不能接得住）在本引擎里
  一条都还没写。

**为什么不在本 PR 硬做**（P1：不留半成品，也不为凑齐清单塞一版猜的）：模式建议的判据不能凭空发明——
它要读的是 `anchorIds` 的跨镜复现、锚点是否已有定妆图产物（`workbench-fixation-planner` 那条链）、以及
`archetype.slots` 的 `characterIndexed` / `min|max` 能不能接住这些参考图。这三样各自都有自己的真相源，
接错一处就是「Agent 建议你用参考图模式，结果那个模式根本没有 image_ref 槽」。硬塞进本 PR 只会得到一版
靠猜的规则，违反 D3（论断要有据）。

**登记为下一刀**：见 §5 新增的 **P4 模式选择建议**——判据设计单独出方案（含跨镜锚点复现的定义、
「有没有定妆图」的查法、模式槽位可达性判定），拍板后再写码。本 PR 的引擎与面板已经为它留好形状：
再加一类 proposal 就能进同一条「逐条采纳 → 自动重查」的通路，不需要改 IPC 契约或面板骨架。
...
- **P4 模式选择建议（2026-09-07 登记，下一刀）**：让引擎除「合并/拆条」外再产一类 `ModeProposal`——
  跨镜同角色 → 建议改走 `character`/`image_ref` 模式（人脸/服装不乱变），同场承接 → 建议首尾帧。
  **先出判据方案再写码**：要定义「跨镜锚点复现」怎么算、锚点有没有定妆图产物怎么查、
  `archetype.slots`（`characterIndexed` / `min|max`）能不能接住这些参考图；三样各有真相源，猜一处就会
  建议一个根本没有 `image_ref` 槽的模式。验收：契约用例覆盖「同角色跨 3 镜 → 建议参考图模式」
  「该模式无对应槽位 → 不建议且说明原因」；面板复用现有「逐条采纳 → 自动重查」通路，不改 IPC 契约。
